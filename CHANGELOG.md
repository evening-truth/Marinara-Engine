...
- Fixed Seedance 2.0 local avatar references so Marinara sends local call-video reference frames as data URIs when no `VIDEO_REFERENCE_PUBLIC_BASE_URL` HTTPS endpoint is configured, allowing compatible Seedance relays to try image-to-video from local installs instead of failing before the request is sent.
